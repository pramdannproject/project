...
const filesDir = path.resolve(__dirname, "../..", "files"); // Resolusi path absolut ke ../files
...
const storage = multer.diskStorage({
  destination: async function (req, file, cb) {
    try {
      ...
      if (parentId) {
        const parentFolder = await prisma.fileNode.findUnique({
          where: { id: parentId },
        });
...
router.delete("/files/:id", async (req, res) => {
  ...
  try {
    ...
    // Jika ini adalah folder, kita hapus folder beserta isinya
      ...
      await deleteFolderRecursively(fileNode.path);
...
router.post("/folders", async (req, res) => {
  ...
  try {
    ...
    const existingFolder = await prisma.fileNode.findFirst({
      where: {
        name: name,
        parentId: parentId || null,
        type: "FOLDER",
    ...
    const folderNode = await prisma.fileNode.create({
      data: {
        name,
        type: "FOLDER",
        path: folderPath,
...
router.put("/files/:id", async (req, res) => {
  ...
  try {
    // Dapatkan FileNode yang akan diubah
    const fileNode = await prisma.fileNode.findUnique({
      where: { id: id },
      include: { parent: true },
    });
    ...
    const existingNode = await prisma.fileNode.findFirst({
      where: {
        parentId: fileNode.parentId || null,
        name: newName,
        type: fileNode.type, // Pastikan tipe sama (FILE/FOLDER)
      },
    });

    if (existingNode) {
      return res.status(400).json({ error: `A ${fileNode.type.toLowerCase()} with the name '${newName}' already exists in this folder` });
    }

    // Tentukan path baru
    const parentPath = fileNode.parent ? fileNode.parent.path : filesDir;
    const newPath = path.join(parentPath, newName);

    // Rename di filesystem
    await fs.promises.rename(fileNode.path, newPath);
    ...
    if (fileNode.type === "FOLDER") {
      ...
      const descendants = await getAllDescendants(fileNode.id);
      ...
      const updatePromises = descendants.map(async (descendant) => {
        ...
        const relativePath = path.relative(fileNode.path, descendant.path);
    ...
    const updatedFileNode = await prisma.fileNode.update({
      where: { id: id },
