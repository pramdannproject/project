...
const DefectOperator = () => {
  ...
  useEffect(() => {
    const fetchFiles = async () => {
      ...
      try {
        const authHeader = Cookies.get("userAuth") || "";
        const response = await fetch(
          `https://${HTTPSAPIURL}/api/manage/gentani/files-tree?parentId=${parentId}`,
          {
            method: "GET",
            headers: {
              "Content-Type": "application/json",
  ...
  const submitAddFolder = async () => {
    ...
    try {
      const authHeader = Cookies.get("userAuth") || "";
      const response = await fetch(
        `https://${HTTPSAPIURL}/api/manage/gentani/folders`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
  ...
  const uploadFileWithProgress = (file: File) => {
    return new Promise<void>((resolve, reject) => {
      ...
      xhr.open(
        "POST",
        `https://${HTTPSAPIURL}/api/manage/gentani/files?parentId=${parentId}`,
        true,
      );
  ...
  const confirmDelete = async () => {
    ...
    try {
      const authHeader = Cookies.get("userAuth") || "";
      const response = await fetch(
        `https://${HTTPSAPIURL}/api/manage/gentani/files/${deleteTarget.id}`,
        {
          method: "DELETE",
          headers: {
            "Content-Type": "application/json",
  ...
  const submitEdit = async () => {
    ...
    try {
      const authHeader = Cookies.get("userAuth") || "";
      const response = await fetch(
        `https://${HTTPSAPIURL}/api/manage/gentani/files/${editTarget.id}`,
        {
          method: "PUT",
          headers: {
            "Content-Type": "application/json",
